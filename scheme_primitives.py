...
class PrimitiveProcedure:
    ...
    def __init__(self, fn, use_env=False, name='primitive'):
        self.name = name
        self.fn = fn
        self.use_env = use_env  #indicates whether or not this primitive procedure will expect the current environment to be passed in as the last argument (该procedure是否考虑env)
...
def primitive(*names):
    ...
    def add(fn):
        ...
        for name in names:
            _PRIMITIVES.append((name,proc)) #[("eq?", proc), ("equal?, proc")]
        return fn
    return add
...
@primitive("boolean?")
def scheme_booleanp(x): #x是boolean类型就返回True
