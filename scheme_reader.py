...
class Pair:
    ...
    def __getitem__(self, k):  #you ignored this method at first. You should always keep DS in mind.
        if k < 0:
...
def scheme_read(src):
    ...
    (car (quote (1 2)))
    """
    if src.current() is None:  #next item
        raise EOFError
    val = src.pop()
    if val == "nil":
        return nil  #nil instance
    elif val not in DELIMITERS:
        return val  #some string or number or + - * /
    elif val == "'":
        "*** YOUR CODE HERE ***"
        return Pair('quote', Pair(scheme_read(src), nil)) #operator是'quote', quote的operand(sub-expression)始终只有一个. keep expression tree in mind
    elif val == "(":
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            "*** YOUR CODE HERE ***"
            src.pop()  #pop "."
            v = scheme_read(src) #read one additional expression
            if src.pop() != ")":  #pop ")"
                raise SyntaxError("Expected one element after .")
            else:
                return v
        else:
            first = scheme_read(src) #consumes one token
            rest = read_tail(src)
