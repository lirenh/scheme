...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:  #special forms. if, and etc.
        result = SPECIAL_FORMS[first](rest, env)
    else:
        procedure = scheme_eval(first, env)  #procedures(opreator)
        args = rest.map(lambda operand: scheme_eval(operand, env)) #在当前的env下,递归eval rest, 得到args(opreands)
        # arg存在Pair里, 形式是well-formed Scheme list
...
def scheme_apply(procedure, args, env):
    # apply two kinds of procedure: primitive proc & lambda
    """Apply Scheme PROCEDURE to argument values ARGS in environment ENV."""
    if isinstance(procedure, PrimitiveProcedure):
        return apply_primitive(procedure, args, env)
    elif isinstance(procedure, UserDefinedProcedure):  #user defined proc(define, lambda), make new frames
...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    4
    """
    # 上面的plus: 把'plus' bind to "+" 所绑定的加法函数
    args = list(args_scheme_list) # Convert a Scheme list to a Python list (has __getitem__)
    "*** YOUR CODE HERE ***"
...
def make_call_frame(procedure, args, env):
    ...
    if isinstance(procedure, MuProcedure):
        environment = env  #dynamic scope
    else:
        environment = procedure.env  #lexical scope
    return environment.make_child_frame(procedure.formals, args)
...
class Frame:
    ...
    def __init__(self, parent):
        ...
        self.bindings = {}  #access through lookup & define
        self.parent = parent
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Number of formals and vals do not match.")
    ...
    def define(self, symbol, value):
        #always bind a symbol to a value in the current value
...
def do_define_form(expressions, env):
    ...
    #at least 2 elements following 'define'
    target = expressions[0]  #e.g. (define (f x) (+ x 1)) / (define v (+ 1 2)) / (define f (lambda (x) (+ x 1)))
    if scheme_symbolp(target):  # variable assignment
        check_form(expressions, 2, 2)
        "*** YOUR CODE HERE ***"
        env.define(target, scheme_eval(expressions[1], env))
        return target
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        #Pair means we should find function name and formal param.
        #then create LambdaProcedure
        "*** YOUR CODE HERE ***"
        formals = target.second
        func = LambdaProcedure(formals, expressions.second, env)
        env.define(target.first, func)  #bind func name to func
        return target.first
    else:
...
def do_lambda_form(expressions, env):
    """Evaluate a lambda form. Return a LambdaProcedure instance."""
    check_form(expressions, 2)  #lambda 后面跟着两个exp, 第一个是formals(scheme list), 第二个是body(scheme list)
...
def make_let_frame(bindings, env):
    ...
    for bind in bindings:
        if not scheme_symbolp(bind.first):
            raise SchemeError("only symbol can be bound")
        check_form(bind, 2, 2)
        frame.define(bind.first, scheme_eval(bind[1], env))
    return frame
...
def check_formals(formals):
    ...
    while cur is not nil:
        if not scheme_symbolp(cur.first):
            raise SchemeError(str(cur.first) + " is not a scheme symbol.")
        cur = cur.second
